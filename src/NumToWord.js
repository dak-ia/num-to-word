...
const toSi = (num) => {
    ...
    if (numArray.integer.length <= 3) {
        ...
        if (integerArray.length == 2) {
            return ((Math.round(Number(integerArray[0] + integerArray[1] + "." + numArray.decimal))) / 1000).toString() + siSymbol[0];
        } else {
            return ((Math.round(Number(integerArray[0] + integerArray[1] + "." + integerArray[2]))) / 1000).toString() + siSymbol[integerArray.length - 2];
        }
    }
}
